...
def video_to_optical_flow(
    src_path, dest_path, compute_method="RAFT", output_format="mp4"
):
...
def main():
    ...
    parser.add_argument("src_path", type=Path, help="Source video path")
    parser.add_argument(
        "dest_path", type=Path, help="Destination path for optical flow video or images"
    )
    parser.add_argument(
        "-m",
        "--compute_method",
    ...
    video_to_optical_flow(
        args.src_path, args.dest_path, args.compute_method, args.output_format
    )
