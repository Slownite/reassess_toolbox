...
# class AudioStream:
#     def __init__(self, filepath, chunk_duration=1.0, sr=None):
#             """
#             Initialize an AudioStream object to handle streaming audio data in chunks from a file.

#             Args:
#                 filepath (str): Path to the audio file.
#                 chunk_duration (float): Duration of each chunk in seconds.
#                 sr (int, optional): Sampling rate to use for loading the audio. If None, use the file's original sample rate.
#             """
#             self.filepath = filepath
...
#                 self.total_samples = f.frames
#                 self.total_duration = self.total_samples / self.sr

#     # def __getitem__(self, index):
#     #     """
#     #     Get the audio data for a specific index from the video stream.

#     #     Args:
#     #         index (int): The index of the video chunk from which to extract audio.

#     #     Returns:
#     #         np.ndarray: The audio data extracted from the video chunk.
#     #     """
#     #     # Retrieve the video chunk using the VideoStreamer
#     #     video_clip = mpy.VideoFileClip(self.video_streamer.video_paths[index])  # Assuming the video path can be accessed directly
#     #     audio_clip = video_clip.audio
#     #     audio_data = audio_clip.to_soundarray(fps=44100)  # Extract audio as an array
#     #     video_clip.close()
