...
*** Contraintes
- Tout le code doit être contenu dans un seul script.
- Si je choisis RAFT le code doit détecter si jùqi une carte graphique si tels est le cas il faut l'utilisé sinon utilisé le cpu.
*** Conseil
...
L'objectif est que puisse apprendre à lire un papier scientifique est d'être capable de l'implémenter en Pytorch
